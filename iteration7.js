//Crea una función que reciba por parámetro un array y el valor que desea comprobar que existe
// dentro de dicho array - comprueba si existe el elemento, en caso que existan nos devuelve un
// true y la posición de dicho elemento y por la contra un false.

const nameFinder = [
  "Peter",
  "Steve",
  "Tony",
  "Natasha",
  "Clint",
  "Logan",
  "Xabier",
  "Bruce",
  "Peggy",
  "Jessica",
  "Marc",
];
function finderName(param, param2) {
  for (const iterator of param) {
    if (iterator === param2) {
      return true + " Ocupa la posición: " + param.indexOf(iterator);
    }
  }

  return false + " No existe ningún elemento con este nombre";
}

console.log(finderName(nameFinder, "Bruce"));
